...
#SELECT -- Returns a list of users based on a common city_ID
def user_by_city_ID(cityID):
    cnx = xenobase_mysql_connector()
    cursor = cnx.cursor(buffered = True)
    query = ("SELECT User_ID, Last_Name, First_Name, Street_Name, City_Name, " +
              "State_Province, Country_Name FROM Users INNER JOIN Address ON Address.Address_ID = Users.Address_Address_ID " +
              "INNER JOIN City ON City.City_ID = Address.City_City_ID " +
              "INNER JOIN Country ON Country.Country_ID = City.Country_Country_ID " +
              "Where City.City_ID = {};".format(cityID))
    cursor.execute(query)
    result = cursor.fetchall()
    ...
    for row in result:
        result_row = []
        result_row.append("User_ID:" + str(row[0]))
        result_row.append("Last_Name:" + str(row[1]))
        result_row.append("First_Name: " + str(row[2]))
        result_row.append("Street_Name: " + str(row[3]))
        result_row.append("City_Name: " + str(row[4]))
        result_row.append("State/Province: " + str(row[5]))
        result_row.append("Country_Name: " + str(row[6]))
        clean_result.append(result_row)
...
@app.route('/get_users_by_city_ID', methods = ['GET'])
def get_users_by_city_ID():
    cityID = request.args.get('cityID')
    response = user_by_city_ID(cityID)
    return jsonify(response), 200
